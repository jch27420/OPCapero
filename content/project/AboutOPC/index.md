...
<figure>
<img src="images/PaleoFireData_EasternNA4.jpg" alt="" width="700px"/>

<figcaption><b>Figure 2: Charcoal Data: History of Fire in Eastern North America.</b> The importance of <span id="Red">fire</span> to the development of grasslands is evident in the region's sedimentary charcoal record. Like pollen and phytoliths, sedimentary charcoal is a useful proxy that can be used to reconstruct past fire regimes. <b>(A)</b> Sedimentary charcoal records from 57 sites (dots on map) in eastern North America were analyzed using the <b>paleofire</b> R package (<b><a class="one" href="https://www.paleofire.org/">Global Charcoal Database</a></b>).<b><sup>79</sup></b> <b>(B)</b> Statistical analysis of the charcoal record was carried out as previously described.<b><sup>79,80</sup></b> Briefly, sites were selected based on location (latitude: 35&deg; to 60&deg;, longitude: -100&deg; to -40&deg;), number of geochronological controls (i.e. &ge; one per 2500 years) and number of samples (n &ge; 20). The charcoal records were standardized using a <u>three-step</u> transformation process: <b>(i)</b> mini-max data re-scaling (i.e. x&prime;<sub>&ii;</sub> = [x<sub>&ii;</sub> - x<sub>min.</sub>]/[x<sub>max</sub> - x<sub>min</sub>], where x&prime;<sub>&ii;</sub> is the min-max transformed value of the &ii;<sup><i>th</i></sup> sample and x<sub>max</sub> and x<sub>min.</sub> are the maximum and minimum values from the site); <b>(ii)</b> the re-scaled data was then Box-Cox transformed<b><sup>81</sup></b> to '<i>normalize</i>' the distribution of the data (i.e. type of power transformation used to homogenize the variance and correct for skewness); and <b>(iii)</b> a final re-scaling of the transformed data to Z-scores was performed (i.e. x&prime;<sub>&ii;</sub> = [x<sub>&ii;</sub> - x<sub><i>mean</i></sub>]/[x<sub><i>sd</i></sub>], where x&prime;<sub>&ii;</sub> is re-scaled value of the &ii;<sup><i>th</i></sup> sample and x<sub>mean</sub> and x<sub><i>sd</i></sub> are the sample mean and standard deviation). Next, the composite mean (solid line) of the data was determined using a data-binning procedure that involved Bootstrap re-sampling (n = 1000 iterations). The binned (1,000 year bins) charcoal series was also used to calculate the 95% and 97.5% confident intervals (<b>CI</b>, dashed lines). <b>(C)</b> A '<i>moving</i>' Bootstrap analysis was also carried to test the significance of local '<i>minima</i>' and '<i>maxima</i>' in the composite time series shown above (plot <b>2B</b>). This method selects (with replacement) random <u>overlapping blocks</u> of data to reconstruct a new charcoal series. This new series is then used to estimate the <b>CI</b> (dashed lines, 99.5%, 97.5%, 95%, 5%, 2.5% and 0.5%) around the composite mean (black line). Both positive and negative anomalies appear within the composite mean time series (highlighted with '<i>stars</i>'). The two positive anomalies between 9,000 and 10,000 <b>ya</b> and around 8,000 <b>ya</b> have composite mean values that exceed the 99.5% and 97.5% <b>CI</b>, suggesting that they are significantly different when compared to the 11,000 year linear trend. The relatively narrow confidence intervals for these two positive anomalies (plot <b>2B</b>) suggests that all 57 sites from across the continent responded in a similar manner (i.e. '<i>spatially synchronous</i>')<b><sup>82</sup></b> indicating that climate was the likely driving force behind these events. The negative anomaly around 11,000 <b>ya</b> has a relatively wide <b>CI</b> for its composite mean value (see <b>Fig. 2B</b>), which is likely due to the small sample size (n = 188, pie chart). However, the second negative anomaly (i.e. 6,000-7,000 <b>ya</b>, n = 879) did not shared the same limitations as the first one, suggesting that all of the sampled sites responded in a similar manner (i.e. likely climate-driven event).
...
**82.** Swetnam T.W. (1993). Fire history and climate change in giant sequoia groves. Science 262(5135):885-889. 
